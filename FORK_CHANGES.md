...
| `dsh-catppuccin@^0.2.3` | 已卸载（2026-08-31，pnpm remove + bundles 移除） | Catppuccin 主题插件。0.2.3 即 npm 最新（2026-08-29 发布，早于 0.1.2 适配潮），产物仍 require 已移除的 `dsh-client-runtime/client`（其 devDeps 未声明 `@deepseek-ai/*`，require 系构建期从 hoisted 依赖外部化）——同 @neystan 套件根因，无升级路径，等作者适配 0.1.2。 |
