...
| `@linxin666/dsh-client-ui-task-board@^0.3.6` | 已卸载（2026-08-31，pnpm remove + bundles 移除） | 另一作者的同名任务板，清空社区插件后从市场装的，0.3.6 与 @neystan 套件同根因（inject 引用已移除的 `dsh-client-runtime` 等，构建目标 0.1.1-rc.2）。**注意：该作者 2026-08-31 已发 0.3.10 适配版**（`engines.dsh: >=0.1.2-alpha.1`，inject 换成 `dsh-client-ui-renderer`/`dsh-api-*-controller`/`dsh-api-remotes` 等 0.1.2 新内部包，devDeps `0.1.2-alpha.2`）——想用任务板装 0.3.10，装前仍建议核对 inject 目标在部署树里存在。 |
